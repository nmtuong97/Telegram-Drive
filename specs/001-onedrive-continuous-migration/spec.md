...
- **FR-007**: Hệ thống PHẢI hỗ trợ lọc file theo định dạng dùng **glob patterns** (không phải regex), áp dụng trên relative path từ thư mục gốc nguồn. Matching **case-insensitive** để hành vi nhất quán giữa Windows/macOS/Linux. Path separator nội bộ: `/`. Include list rỗng = include tất cả. Exclude pattern luôn thắng include. Default exclusions: `~$*`, `*.tmp`, `*.partial`, `*.crdownload`, `desktop.ini`, `Thumbs.db`, `.DS_Store`. UI phải hiển thị validation error với pattern không hợp lệ. Chỉ xử lý items có `parentReference.path` khớp chính xác `source_folder_path` — bỏ qua subfolder items.
...
- **FR-014**: Hệ thống PHẢI áp dụng pacing giữa các lần upload: file ≤ 1 MiB: ngẫu nhiên 45–90 giây; file > 1 MiB và ≤ 10 MiB: ngẫu nhiên 30–60 giây; file > 10 MiB: ngẫu nhiên 15–30 giây. Burst guard: 10 file ≤ 1 MiB liên tiếp → nghỉ 10 phút; 20 file bất kỳ trong 1 giờ → nghỉ 5 phút. Không tuyên bố đây là hạn mức chính thức của Telegram.
...
- **FR-030**: Hệ thống PHẢI quản lý trạng thái job theo các trạng thái: `created → preflight → running ⇄ paused`, các trạng thái chờ: `waiting`, `graph_cooldown`, `telegram_cooldown`, `low_disk`, `auth_required`; trạng thái chuyển tiếp: `stopping`; trạng thái kết thúc: `completed`, `cancelled`, `fatal`. Các trạng thái chờ ngang hàng với `running` trong state machine (transition `(Running, Waiting)`, `(Waiting, Running)`, v.v.) — không phải enum lồng.
